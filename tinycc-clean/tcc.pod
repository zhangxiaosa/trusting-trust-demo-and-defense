...
The bound checking code is not included in shared libaries. The main executable should always be compiled with the B<-b>.
...
Note: B<-b> is only available on i386 (linux and windows) and x86_64 (linux and windows) for the moment.
...
A function C< int tcc_backtrace(const char *fmt, ...); > is provided
